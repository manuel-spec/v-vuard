...
public static class ShieldDroneBehavior
{
    public static BehaviorNode Build(Blackboard board)
    {
        return new Selector(
            new Sequence(
                ...
                new Cooldown(new ActionNode((w, e, dt) => Attack(w, e, dt, board)), 0.95f)
            ),
    ...
    private static BehaviorStatus Patrol(World world, Entity entity, float dt, Blackboard board)
    {
        ...
        v.Value = DeterministicMath.Quantize(new Vector2(dir * 73f, v.Value.Y));
