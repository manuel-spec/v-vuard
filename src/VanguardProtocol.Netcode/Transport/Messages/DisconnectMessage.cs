...
{
    public DisconnectMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out DisconnectMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
    ...
    public static DisconnectMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new DisconnectMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new DisconnectMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<DisconnectMessage> batch)
    {
        var offset = 0;
