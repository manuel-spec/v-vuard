...
public readonly struct ChecksumMessage
{
    public ChecksumMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out ChecksumMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
        message = new ChecksumMessage(
    ...
    public static ChecksumMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new ChecksumMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new ChecksumMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<ChecksumMessage> batch)
    {
        var offset = 0;
