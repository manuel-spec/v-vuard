...
public readonly struct ResumeMessage
{
    public ResumeMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out ResumeMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
    ...
    public static ResumeMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new ResumeMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new ResumeMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<ResumeMessage> batch)
    {
        var offset = 0;
