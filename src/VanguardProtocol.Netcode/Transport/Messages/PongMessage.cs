...
{
    public PongMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out PongMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
    ...
    public static PongMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new PongMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new PongMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<PongMessage> batch)
    {
        var offset = 0;
