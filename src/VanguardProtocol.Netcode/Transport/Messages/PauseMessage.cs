...
public readonly struct PauseMessage
{
    public PauseMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out PauseMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
    ...
    public static PauseMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new PauseMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new PauseMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<PauseMessage> batch)
    {
        var offset = 0;
