...
public readonly struct HelloAckMessage
{
    public HelloAckMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out HelloAckMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
    ...
    public static HelloAckMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new HelloAckMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new HelloAckMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<HelloAckMessage> batch)
    {
        var offset = 0;
