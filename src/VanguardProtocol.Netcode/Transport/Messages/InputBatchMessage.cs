...
public readonly struct InputBatchMessage
{
    public InputBatchMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out InputBatchMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
        message = new InputBatchMessage(
    ...
    public static InputBatchMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new InputBatchMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new InputBatchMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<InputBatchMessage> batch)
    {
        var offset = 0;
