...
{
    public ChatMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out ChatMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
    ...
    public static ChatMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new ChatMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new ChatMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<ChatMessage> batch)
    {
        var offset = 0;
