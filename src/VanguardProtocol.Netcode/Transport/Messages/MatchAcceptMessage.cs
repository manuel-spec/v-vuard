...
public readonly struct MatchAcceptMessage
{
    public MatchAcceptMessage(ulong frame, uint token, ushort payload)
    {
        Frame = frame;
        Token = token;
        Payload = payload;
    ...
    public static bool TryRead(ReadOnlySpan<byte> src, out MatchAcceptMessage message)
    {
        message = default;
        if (src.Length < Size)
            return false;
        message = new MatchAcceptMessage(
    ...
    public static MatchAcceptMessage[] CreateBatch(int count, ulong startFrame, uint tokenSeed)
    {
        var arr = new MatchAcceptMessage[count];
        for (var i = 0; i < count; i++)
            arr[i] = new MatchAcceptMessage(startFrame + (ulong)i, tokenSeed + (uint)i, (ushort)(i * 17));
        return arr;
    }

    public static int WriteBatch(Span<byte> dest, ReadOnlySpan<MatchAcceptMessage> batch)
    {
        var offset = 0;
